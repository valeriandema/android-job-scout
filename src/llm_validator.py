...
def build_prompt(job: Job, target_regions: list[str]) -> tuple[str, str]:
    ...
    del target_regions  # kept for backwards compatibility; rules are hardcoded
    system = (
        "You classify whether a job posting is open to a fully-remote candidate "
        "based in Bulgaria or Ukraine. "
        "Use ONLY the text provided; do not use outside knowledge. "
        "Default: eligible=false.\n"
        "\n"
        "Set eligible=true ONLY if AT LEAST ONE of these is true:\n"
        "(a) The text explicitly names Bulgaria or Ukraine as a country where "
        "hiring is allowed.\n"
        "(b) The text uses an unrestricted umbrella term that necessarily "
        "includes Bulgaria and Ukraine — e.g. 'worldwide', 'anywhere', "
        "'global', 'remote anywhere', 'anywhere in the world', or a bare "
        "'Europe' / 'EU' / 'European Union' / 'EEA' / 'EMEA' with NO "
        "qualifying country restriction nearby.\n"
        "\n"
        "Set eligible=false if ANY of these is true:\n"
        "- The text enumerates a specific list of allowed countries and "
        "neither Bulgaria nor Ukraine appears on that list. This applies "
        "EVEN IF every country on the list is European. For example, "
        "'France, Germany, Italy, Spain, and Portugal' -> eligible=false, "
        "because Bulgaria and Ukraine are not on the list.\n"
        "- Restricts to US / Canada / LATAM / APAC, or requires US/Canadian/UK "
        "work authorization.\n"
        "- Names a specific timezone band that excludes EET (UTC+2/+3) — e.g. "
        "'US timezone', 'EST/PST overlap', 'Americas hours only'.\n"
        "- Is silent on geography or gives no geographic signal at all.\n"
        "\n"
        "Conflict rule: if a posting contains BOTH an umbrella term AND a "
        "specific country enumeration, the enumeration wins — treat the "
        "umbrella as marketing copy. 'Remote in Europe (France, Germany, "
        "Spain)' -> eligible=false.\n"
        "\n"
